...
(function ($, window, document, undefined) {
    ...
    function _modal_structure(config, ok, cancel) {
        ...
        //console.log(config.backdrop);
        if (config.backdrop) {
            modal.setAttribute('aria-hidden', true);
            modal.setAttribute('data-backdrop', config.backdrop);
        }
    ...
    var _STYLE = {info: 'bg-aqua', correct: 'bg-green', warn: 'bg-yellow', error: 'bg-red'};

    function _alert(args, type) {
        var cfg = modal_params.apply(this, args);
        cfg.title = cfg.title || _TITLE[type];
        cfg.style = cfg.style || _STYLE[type];
    ...
    var _win_config = {winId: 'modal-win', backdrop: true, keyboard: true, width: 900};
    ...
    //add by alonxiong fix the wyhtml5 problem: wyhtml has the modal class,make the scrollbar hidden
